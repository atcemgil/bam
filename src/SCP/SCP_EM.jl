...
function standard_EM(X::Array{ℜ,3}, X⁺::Array{ℜ,2}, R::Ƶ ; μ::ℜ=3.5, γ::ℜ=0.1, EPOCHS::Ƶ=1, ϵ::ℜ=1e-16) where {ℜ<:Real, Ƶ<:Int}
    I::Ƶ, J::Ƶ, K::Ƶ = size(X)
    ...
    for eph=1:EPOCHS
        ...
        log_θ_IKR .= log.(max.(S_IKR.+α_IKR.-1.0, ϵ))
        log_θ_JKR .= log.(max.(S_JKR.+α_JKR.-1.0, ϵ))

        log_θ_R .-= logsumexp(log_θ_R)
        log_θ_KR .-= logsumexp(log_θ_KR;dims=1)
        log_θ_IKR .-= logsumexp(log_θ_IKR;dims=1)
        log_θ_JKR .-= logsumexp(log_θ_JKR;dims=1)
    end

    return X_full, log_λ, (log_θ_R, log_θ_KR, log_θ_IKR, log_θ_JKR)
end

function dual_EM(X::Array{ℜ,3}, X⁺::Array{ℜ,2}, R::Ƶ; μ::ℜ=3.5, γ::ℜ=0.1, EPOCHS::Ƶ=1, ϵ::ℜ=1e-16) where {ℜ<:Real, Ƶ<:Int}
    I::Ƶ, J::Ƶ, K::Ƶ = size(X)
    ...
    return X_full, S⁺, (S_R, S_KR, S_IKR, S_JKR)
end
